...
namespace Admissions.Data
{
    public static class Queries
    {
        ...
        {
            tenantId = tenantId ?? Environment.Settings.TenantId;
            string sql = string.Format("SELECT TOP 1 Description FROM dbo.{0} WHERE Code LIKE '{1}'" +
                                       " and TenantID = '{2}'", entityName, code, tenantId);
        ...
        public static string GetConfiguredConductLabel(string conductType, int? tenantId = null)
        {
            ...
            string sql = string.Format(@"
                ...
                WHERE aec.TenantID = {0}", tenantId, conductType);
        ...
        public static Guid GetCriteriaType(string CriteriaID, string CriteriaType, int? tenantId = null)
        {
            tenantId = tenantId ?? Environment.Settings.TenantId;
            ...
            string sql = String.Format(
                @"SELECT TOP 1 {0}
                 FROM dbo.[{1}] 
                WHERE TenantID = '{2}'
                  AND IsVisible = 1", CriteriaID, CriteriaType, tenantId);
            return DataAccessHelpers.GetValue<Guid>(sql);
        }
    }
}