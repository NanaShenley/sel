...
using PageObjectModel.Components.HomePages;
...
namespace PageObjectModel.Helper
{
    public static class SeleniumHelper //ElementAccessor
    {
        ...
        public static void SetText(this IWebElement element, string value)
        {
            ...
            {
                Console.WriteLine("Can't sendkeys '" + value + "' to element - StaleElementReferenceException:" + serx.ToString());
            }
            catch (Exception ex)
            {
            ...
            bool timeOnly = false)
        {
            ...
            if (!String.IsNullOrEmpty(value))
            {
                ...
                {
                    element.SetText(
                        DateTime.ParseExact(value, "M/d/yyyy", CultureInfo.InvariantCulture).ToString(elementFormat));
                }
                else
                {
                    if (timeOnly)
                    ...
                    {
                        element.SetText(
                            DateTime.ParseExact(value, "M/d/yyyy h:mm tt", CultureInfo.InvariantCulture)
        ...
        public static void SetDateTimeByJS(this IWebElement element, string value, bool formatTime = false)
        {
            ...
            {
                element.SetText(
                    DateTime.ParseExact(value, "M/d/yyyy h:mm tt", CultureInfo.InvariantCulture).ToString(elementFormat));
            }

            element.SendKeys(Keys.Tab);
        }

        private static IWebElement GetParentElement(By parentElementBy)
        {
            return ElementRetriever.FindElementSafe(WebContext.WebDriver, parentElementBy);
        }
        ...
        public static void SetDateTimeByJS(By by, string value, bool formatTime = false)
        {
            ...
            ElementRetriever.FindElementSafe(WebContext.WebDriver, by).Click();

            string elementFormat = Regex.Replace(ElementRetriever.FindElementSafe(WebContext.WebDriver, by).GetAttribute("data-date-validator-format"), "a|A", "tt");

            ElementRetriever.FindElementSafe(WebContext.WebDriver, by).ClearText();

            if (!formatTime)
            {
                ElementRetriever.FindElementSafe(WebContext.WebDriver, by).SetText(
                    DateTime.ParseExact(value, "M/d/yyyy", CultureInfo.InvariantCulture).ToString(elementFormat));
            }
            else
            {
            ...
            ElementRetriever.FindElementSafe(WebContext.WebDriver, by).SendKeys(Keys.Tab);
        }
        ...
        {
            SeleniumHelper.WaitForElementClickableThenClick(By.CssSelector(SeleniumHelper.AutomationId("task_menu")));
	        SeleniumHelper.FindElementWithOutTimeout(SimsBy.Id("task-menu-search"));
        ...
        public static string GetMonthsAndYears(this string dob)
        {
            ...
            return string.Format("{0} Years {1} Month(s)", years, months);
