...
namespace POM.Components.Staff
{
    public class StaffRecordPage : BaseComponent
    {
        ...
        public void SelectAddressesTab()
        {
            ...
            //_addressesTab.Click();
            //Actions builder = new Actions(WebContext.WebDriver);
            //builder.Click(_addressesTab);
            //builder.Build().Perform();

            //_addressesTab.SendKeys(OpenQA.Selenium.Keys.Enter);

            //IWebElement el = WebContext.WebDriver.FindElement(By.CssSelector("[data-automation-id='section_menu_Addresses']"));
            //el.Click();

            if (SeleniumHelper.Get(By.CssSelector("[data-automation-id='section_menu_Addresses']")).Displayed)
            {
                SeleniumHelper.FindElement(By.CssSelector(SeleniumHelper.AutomationId("section_menu_Addresses"))).Click();
        ...
        private IWebElement _addEmail;


        public void ClickAddEmailId()
        {
            _addEmail.ClickByJS();
            Wait.WaitForAjaxReady(By.CssSelector(".locking-mask-loading"));
        }
        ...
        {
            Thread.Sleep(2000);
            IWebElement el = SeleniumHelper.FindElement(By.CssSelector(SeleniumHelper.AutomationId("add_address_button")));
            el.Click();
        ...
        public static DataPackage CreateStaffRecord(out Guid staffId, string forename, string surname, DateTime startDate)
        {
            Guid serviceRecordId;

            DataPackage testData = new DataPackage();

            testData.AddData("Staff", IDCDataPackageHelper.GenerateStaff(out staffId, surname, forename: forename));
            testData.AddData("StaffServiceRecord", IDCDataPackageHelper.GenerateServiceRecord(out serviceRecordId, staffId, startDate));
