...
namespace SharedComponents.HomePages
{
    public class TaskMenuBar : BaseSeleniumComponents
    {
        ...
        //private IWebElement taskManagerButton; //= WebContext.WebDriver.FindElement(By.CssSelector("#shell-menu > div > div.shell-menu > div:nth-child(1) > ul > li:nth-child(1) > a"));

        [FindsBy(How = How.LinkText, Using = "School Management")]
        public IWebElement schoolManagementLink;

        [FindsBy(How = How.CssSelector, Using = "[data-automation-id='task_menu']")]
        public IWebElement taskManagerButton;
        ...
        public TaskMenuBar ClickCommunicationTaskMenuBar()
        {
            WaitForTaskMenuBarButton();
            GetMenuBarButton().Click();
            return this;
        ...
        public void ClickSchoolGroupLinkWithoutWait()
        {
             WebDriverWait waiter = new WebDriverWait(WebContext.WebDriver, TimeSpan.FromSeconds(30));
            IWebElement element = waiter.Until<IWebElement>(d =>
            {
                IWebElement link = d.FindElement(By.LinkText("School Groups"));
                return link;
            });
        ...
        public void ClickPupilLinkWithoutWait()
        {
            WebDriverWait waiter = new WebDriverWait(WebContext.WebDriver, TimeSpan.FromSeconds(30));
            IWebElement element = waiter.Until<IWebElement>(d =>
            {
                IWebElement link = d.FindElement(By.LinkText("Pupils"));
                return link;
            });
        ...
        public void ClickPupilRecordsLink(bool senFlag = false)
        {
            WebContext.Screenshot();
            WebDriverWait waiter = new WebDriverWait(WebContext.WebDriver, TimeSpan.FromSeconds(30));
            IWebElement element = waiter.Until<IWebElement>(d =>
            {
                ...
                if(senFlag)
                {
                    try 
                    {
                        link = d.FindElement(By.CssSelector(SeleniumHelper.AutomationId("task_menu_section_pupils_pupil_record")));
                    }
                    catch(Exception)
                    {
                        link = d.FindElement(By.LinkText("Pupil Records"));
                    }
                }
