...
{
    public class LeaverBasicDetailSection
    {
        [FindsBy(How = How.CssSelector, Using = "[data-automation-id='section_menu_Leavers']")]
        private IWebElement _leaverSection;

        [FindsBy(How = How.Name, Using = "LeaverPupilSection.LeaverPupils")]
        private IWebElement _leaverGrid;

        public LeaverBasicDetailSection()
        {
            PageFactory.InitElements(WebContext.WebDriver, this);
        }

        public WebixComponent<OnRollCell> LeaverGrid
        {
            get { return new WebixComponent<OnRollCell>(_leaverGrid); }
        }
