...
{
    public class OnRollPupilExclusionSection
    {
        [FindsBy(How = How.CssSelector, Using = "[data-automation-id='section_menu_On Roll Pupil']")]
        private IWebElement _onrollPupilSection;

        [FindsBy(How = How.Name, Using = "OnRollPupilSection.OnRollPupilExclusions")]
        private IWebElement _onrollExclusionGrid;

        public OnRollPupilExclusionSection()
        {
            PageFactory.InitElements(WebContext.WebDriver, this);
        }

        public WebixComponent<OnRollCell> OnrollExclusionGrid
        {
            get { return new WebixComponent<OnRollCell>(_onrollExclusionGrid); }
        }
        ...
        {
            return OnrollExclusionGrid.RowCount > 0;
        }
