...
using DataExchange.POM.Components.HomePages;
using System;
...
namespace DataExchange.POM.Helper
{
    public static class SeleniumHelper//ElementAccessor
    {
        ...
        public const string NameAttributeFormat = "[name='{0}']";

        public static string NameAttribute(string value)
        {
            return String.Format(NameAttributeFormat, value);
        }
        ...
        public static Dictionary<string, string> GetAllWidget()
        {
            List<IWebElement> allWidget = WebContext.WebDriver.FindElements(By.ClassName("hp-tile-container")).ToList<IWebElement>();
            
            Dictionary<string, string> allWidgetDetails = new Dictionary<string, string>();
            foreach (var widgetSection in allWidget)
            {
                if (string.IsNullOrEmpty(widgetSection.Text) == false)
                {
                    var widgetTitle = widgetSection.FindChild(By.CssSelector(".hp-tile-title")).Text;
                    var widgetDesc = widgetSection.FindChild(By.CssSelector(".hp-tile-desc")).Text;
                    allWidgetDetails.Add(widgetDesc, widgetTitle);
                }
            }
        ...
        public static void ClearText(this IWebElement element)
        {
            ...
            {
                Console.WriteLine("Have an exception when executing 'Send Keys' :" + e.ToString());
            }
        }
        ...
        public static void SetText(this IWebElement element, string value)
        {
            ...
            {
                Console.WriteLine("Can't sendkeys to element" + ex.ToString());
            }
        }
        ...
        public static void EnterForDropDown(this IWebElement element, string value)
        {
            try
            {
                ...
                var inputElement = ElementRetriever.GetOnceLoaded(SimsBy.CssSelector("#select2-drop input.select2-input"));
        ...
        public static void SetDateTime(this IWebElement element, string value, bool formatTime = false, bool timeOnly = false)
        {
            ...
            if (!String.IsNullOrEmpty(value))
            {
                if (!formatTime)
                {
                    element.SetText(DateTime.ParseExact(value, "d/M/yyyy", CultureInfo.InvariantCulture).ToString(elementFormat));
                }
                else
                {
                    if (timeOnly)
                    {
                        element.SetText(DateTime.ParseExact(value, "h:mm tt", CultureInfo.InvariantCulture).ToString(elementFormat));
                    }
                    else
                    {
                        element.SetText(DateTime.ParseExact(value, "d/M/yyyy h:mm tt", CultureInfo.InvariantCulture).ToString(elementFormat));
                    }
                }
            }
            else
        ...
        /// </summary>
        /// <param name="selector">element selector</param>
        public static void WaitForElementClickableThenClick(IWebElement element)
        {
            WebDriverWait wait = new WebDriverWait(WebContext.WebDriver, BrowserDefaults.TimeOut);
        ...
        public static void NavigateBySearch(string itemMenu, bool wait = false)
        {
            ...
            if (taskResult == null)
            {
                ...
                SeleniumHelper.Type(searchTextbox, itemMenu, 1000);
                taskResults = FindElements(SimsBy.CssSelector(".task-menu-dropdown .task-menu-suggestion"));
        ...
        /// <summary>
        /// Log in as a given user. SchoolAdministrator as default.
        /// </summary>
        /// <param name="userType">The user type to login as.</param>
        /// <param name="schoolSelection">show  school selection?</param>
        public static void Login(SeleniumHelper.iSIMSUserType userType = iSIMSUserType.SchoolAdministrator, bool schoolSelection = true, params string[] enabledFeatures)
        {
            switch (userType)
            {
        ...
        public static void ClickFooterTab(string tabName)
        {
            ...
            {
                case "home page":
                    SeleniumHelper.FindElement(By.CssSelector("[data-automation-id='home_page_tab']>a")).ClickByJS();
