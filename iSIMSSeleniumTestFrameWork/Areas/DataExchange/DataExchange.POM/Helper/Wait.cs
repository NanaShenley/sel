    ...
    public static class Wait
    {
        ...
        public static bool WaitForAjaxReady(By ajaxProgressElement)
        {
            ...
            if (ajaxIsComplete)
                ...
                throw new WebDriverTimeoutException(string.Format("1.Waiting for connections to close timed out after {0}ms", timeOut.Milliseconds));
        ...
        public static bool WaitTillAllAjaxCallsComplete()
        {
            Console.WriteLine("Waiting for all Ajax request to complete, if any");
            bool ajaxIsComplete = false;
            Stopwatch waiting = new Stopwatch();
            TimeSpan timeOut = new TimeSpan(0, 0, 90);
            waiting.Start();
            while (waiting.Elapsed <= timeOut)
            {
                string openConnections = (WebContext.WebDriver as IJavaScriptExecutor).ExecuteScript("return jQuery.active").ToString();
            ...
            if (ajaxIsComplete)
                ...
                throw new WebDriverTimeoutException(string.Format("Waiting for all AJAX connections complete. Timed out after {0}ms", timeOut.TotalMilliseconds));
        ...
        /// 
        /// </summary>
        /// <param name="JsPredicate">boolean js predicate. e.g.  return $("[data-section-id = 'toast-container']").find("[role='alert']").length > 0";</param>
        /// <param name="timeOutInSeconds"></param>
        /// <returns></returns>
        public static bool WaitTillConditionIsMet(string JsPredicate, int timeOutInSeconds)
        {
            bool isTrue = false;
            Stopwatch waiting = new Stopwatch();
            waiting.Start();
            ...
            while (waiting.ElapsedMilliseconds <= timeOutInMiliseconds)
            {
                isTrue = (Boolean)jsExecutor.ExecuteScript(JsPredicate);
            ...
            if (isTrue)
                Console.WriteLine("Required condition met after {0}ms.", waiting.ElapsedMilliseconds);
            else
                throw new WebDriverTimeoutException(string.Format("Waiting for condition timed out after {0} seconds", timeOutInSeconds));
