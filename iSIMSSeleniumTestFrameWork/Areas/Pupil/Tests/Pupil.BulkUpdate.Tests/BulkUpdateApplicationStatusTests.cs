...
namespace Pupil.BulkUpdate.Tests
{
    public class BulkUpdateApplicationStatusTests
    {
        ...
        [WebDriverTest(Enabled = true,
             Groups = new[] { PupilTestGroups.ApplicantBulkUpdate.ApplicationStatusGrid,
                             PupilTestGroups.ApplicantBulkUpdate.ApplicationStatusGridMenu,
        ...
        public void Checks_Whether_ApplcationStatus_Save_Dialog_Works_As_Expected()
        {
            ...
            using (new DataSetup(purgeBeforeInsert: false, purgeAfterTest: true, packages: dataPackage))
            {
                ...
                if (
                    !POM.Helper.SeleniumHelper.IsElementDisplayed(
                        POM.Helper.SeleniumHelper.Get(ApplicationStatusSearch.SearchButton)))
                {
