...
namespace Pupil.Admissions.Tests
{
    public class SchoolIntakeTests
    {
        ...
        public void Create_New_Active_School_Intake()
        {
            ...
            var dateOfAdmission = new DateTime(DateTime.Today.Year, 8, 8).ToString("M/d/yyyy");
            ...
            var schoolIntakeName = admissionYear + " - " + admissionTerm + " " + yearGroup + " " + admissionGroupName; 

            #region PRE-CONDITIONS:

            SeleniumHelper.Login(SeleniumHelper.iSIMSUserType.AdmissionsOfficer);
            AutomationSugar.NavigateMenu("Tasks", "Admissions", "School Intakes");
            Thread.Sleep(2000);
            ...
            schoolIntakePage.Name = schoolIntakeName;
            Thread.Sleep(1000);
            ...
            // Verify that a new school intake was created
            Assert.IsNotNull(schoolIntakeTile, "Creating school intake has failed");
            ...
            try
            {
                ...
                Assert.AreNotEqual(null,
                    (admissionGroupTable.Rows.SingleOrDefault(
                        x => x.Name == admissionGroupName && x.DateOfAdmission == dateOfAdmission &&
        ...
        public void Delete_Active_School_Intake()
        {
            ...
            var schoolIntakeName = admissionYear + " - " + admissionTerm + " " + yearGroup.FullName + " " + admissionGroupName;
            ...
            dataPackage.AddSchoolIntake(
                ...
                dateOfAdmission,
                capacity,
                schoolInTakeName: schoolIntakeName);
            ...
            using (new DataSetup(purgeBeforeInsert: false, purgeAfterTest: true, packages: dataPackage))
            {
                ...
                // Verify that a new school intake was created
                Assert.IsNotNull(schoolIntakeTile, "Creating school intake at data setup has failed");
                ...
                try
                {
                    ...
                    Assert.AreEqual(admissionYear, schoolIntakePage.AdmissionYear,
                        "Cancel to delete school intake has failed");
                    Assert.AreEqual(admissionTerm, schoolIntakePage.AdmissionTerm, "Cancel to delete intake has failed");
                    Assert.AreEqual(yearGroup.FullName, schoolIntakePage.YearGroup,
                        "Cancel to delete school intake has failed");
                    Assert.AreEqual(numberOfPlannedAdmission.ToString(), schoolIntakePage.NumberOfPlannedAdmissions,
                        "Cancel to delete school intake has failed");
                    Assert.AreEqual((schoolIntakeName), schoolIntakePage.Name,
                        "Cancel to delete school intake has failed");
                    ...
                    Assert.AreNotEqual(null,
                        (admissionGroupTable.Rows.SingleOrDefault(
                            x =>
                                x.Name == admissionGroupName &&
                                x.DateOfAdmission == dateOfAdmission.ToString("M/d/yyyy") &&
                                x.Capacity == capacity.ToString() && x.Active)), "Creating school intake has failed");
                }
                finally
                {
