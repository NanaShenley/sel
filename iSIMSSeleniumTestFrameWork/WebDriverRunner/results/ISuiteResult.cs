...
namespace WebDriverRunner.results
{
    public interface ISuiteResult
    {
        ...
        List<ITestResult> Get(MethodInfo method);
        List<MethodInfo> GetMethods();

        List<ITestResult> FailedTests { get; }
        List<ITestResult> PassedTests { get; }
        List<ITestResult> SkippedTests { get; }

        List<ITestResult> FailedConfiguration { get; }
        List<ITestResult> PassedConfiguration { get; }
        List<ITestResult> SkippedConfiguration { get; }

        int GetTotalTests();
        int GetTotalConfiguration();
    }
}
